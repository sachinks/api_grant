"""
NSF Awards — CLI Entry Point
------------------------------
Run this script to pull NSF grant awards for a given date and save to Excel.

Usage:
    python scripts/nsf_awards.py                    # defaults to yesterday
    python scripts/nsf_awards.py --date 2025-01-10  # specific date
    python scripts/nsf_awards.py --debug            # verbose logging

Output:
    NSF_YYYYMMDD.xlsx in the current working directory
...
from extractors.nsf import NSFExtractor  # noqa: E402
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Extract NSF grant awards to Excel.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python scripts/nsf_awards.py
  python scripts/nsf_awards.py --date 2025-01-10
  python scripts/nsf_awards.py --date 2025-01-10 --debug
        """,
    )
    ...
    parser.add_argument(
        ...
        default=None,
        metavar="YYYY-MM-DD",
        help="Target date for award date filter (default: yesterday)",
    )
...
def main() -> None:
    ...
    try:
        extractor = NSFExtractor(target_date=target_date)
